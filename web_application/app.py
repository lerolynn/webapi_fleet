...
# Path of user in uploads folder (temp path admin - path changed after login)
app.config["USR_UPLOADS"] = os.path.join(dirname, "static/uploads/admin")
app.config["MAP_UPLOADS"] = os.path.join(dirname, "static/uploads/admin/maps")
app.config["TASK_UPLOADS"] = os.path.join(dirname, "static/uploads/admin/tasks")
...
def run_tasks():
    ...
    # Get tasklist file from database
    cur.execute("SELECT tasklist_file FROM tasklists WHERE user_id=? AND tasklist_name=?", (session["user_id"],tasklist))
    rows = cur.fetchall()
    if len(rows) == 0:
        return apology("Taskfile not in database", 404)

    task_filepath = os.path.join(app.config["TASK_UPLOADS"], rows[0][0])
    print(task_filepath)
    with open(task_filepath) as taskfile:
        taskdata = json.load(taskfile)

    print("Tasklist " + tasklist + " running on map " + map_name + " with robots ", end="")
    print(taskdata)
    return jsonify(taskdata)
...
class GetTasks(Resource):
    def get(self):
        ...
        # Get tasklist file from database
        cur.execute("SELECT * FROM tasklists")
        rows = cur.fetchall()

        task_filepath = os.path.join(app.config["TASK_UPLOADS"], str(rows[0][2]))
        print(task_filepath)
        with open(task_filepath) as taskfile:
            taskdata = json.load(taskfile)
